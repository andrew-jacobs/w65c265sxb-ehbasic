...
;-------------------------------------------------------------------------------
		...
		.BYTE	"XXX",0		; Boot Signature

		SEI			; Disable interrupts
		CLD
		SEC			; Return to emulation mode
		XCE
		JMP	LAB_COLD	; And start BASIC
...
RDBYTE:
		LDA	UIFR		; Any data in the UART buffer?
		AND	#1<<6
		CLC
		BEQ	.DONE		; No
		LDA	ARTD3		; Yes, read it
		SEC
.DONE		RTS
...
WRBYTE:
		PHA
.WAIT		LDA	UIFR		; Ready to transmit again?
		AND	#1<<7
		BEQ	.WAIT
		PLA
		STA 	ARTD3		; Send the character
		RTS
...
LAB_2D93
		LDA	#$00		; get all bits off
		STA	(Itempl)	; clear tested memory byte
...
LAB_CTNM
		...
		BRA	LAB_CKTM	; branch around next instruction
...
LAB_EXIT
		JMP	V_EXIT		; exit back to C02 Monitor Warm start soft vector
...
;
		.word	RDBYTE		; byte in from C02BIOS (no waiting)
		.word	WRBYTE		; byte out from C02BIOS
		.word	SYSLOAD		; null load JMP for EhBASIC
		.word	SYSSAVE		; null save JMP for EhBASIC
		.word	SYSEXIT		; Warm start JMP for C02 Monitor
...
LAB_CTBL
		...
		.word	LAB_EXIT-1	; EXIT		new command (exits to C02 Monitor)
